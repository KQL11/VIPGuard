...
def import_tf_params(tf_mdl_dir, sess):
    """Import tensorflow models from save directory.
...
def get_layer_indices(layer_lookup, tf_layers):
    """Giving a lookup of models layer attribute names and tensorflow variable names,
...
def test_loaded_params(mdl, tf_params, tf_layers):
    """Check each parameter in a pytorch models for an equivalent parameter
...
def compare_model_outputs(pt_mdl, sess, test_data):
    ...
    Arguments:
        pt_mdl {torch.nn.Module} -- Pytorch models.
        sess {tensorflow.Session} -- Tensorflow session object.
        test_data {torch.Tensor} -- Pytorch tensor.
    """
    print('\nPassing test data through TF models\n')
    ...
    print('\nPassing test data through PT models\n')
...
def load_tf_model_weights(mdl, layer_lookup, tf_mdl_dir, is_resnet=True, arg_num=None):
    """Load tensorflow parameters into a pytorch models.

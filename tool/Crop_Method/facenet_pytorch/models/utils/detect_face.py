...
def crop_noresize(img, box, image_size):
    if isinstance(img, np.ndarray):
        img = img[box[1]:box[3], box[0]:box[2]]
        # out = cv2.resize(
        #     img,
        ...
        #     (image_size, image_size)
        # ).byte().squeeze(0).permute(1, 2, 0)
        out = img.permute(2, 0, 1).unsqueeze(0).float()
        out = out.byte().squeeze(0).permute(1, 2, 0)
    else:
        # out = img.crop(box).copy().resize((image_size, image_size), Image.BILINEAR)
...
def extract_face(img, box, image_size=160, margin=0, save_path=None):
    ...
    if image_size is not None:
        ...
        raw_image_size = get_size(img)
        margin = [
            margin * (box[2] - box[0]) / (raw_image_size[0] - margin),
            margin * (box[3] - box[1]) / (raw_image_size[1] - margin),
        ]
        box = [
            int(max(box[0] - margin[0] / 2, 0)),
            int(max(box[1] - margin[1] / 2, 0)),
        ...
        face = crop_noresize(img, box, image_size)

...
def get_multimodal_target_regex(
    ...
    *,
    ...
    another_rejected_modules: List[str] = None,
) -> str:
    # NOTE
    print("The model architecture shows as follows:")
    print(model.model_meta)
    model_arch = get_model_arch(model.model_meta.model_arch)
    modules = []
    if another_rejected_modules is not None:
        rejected_modules = another_rejected_modules
    else:
        rejected_modules = []
    if not freeze_llm:

...
# NOTE: 独立设定的QWEN
class KQ_Qwen2_5VLTemplate(Qwen2VLTemplate):
    version = 'v2_5'
    norm_bbox = 'none'
    placeholder_tokens = ['<|image_pad|>', '<|video_pad|>', '<|face_pad|>']
    special_keys = ['images', 'videos', 'audios', 'objects', 'face', 'face_vip']
    face_token_id = 151665
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        img_token_num = 0
        vip_token_num = 256
        for media_type in ['images', 'videos', 'face']:
            if locals()[media_type]:
                if media_type == 'images':
                    media_token = self.image_token_id
                    media_inputs = processor.image_processor(
                    ...
                    media_token = self.face_token_id
                    img_num = media_inputs['image_grid_thw'].shape[0]
                    if img_num == 1:
                        media_grid_thw = media_inputs['image_grid_thw']
                    elif img_num == 2:
                        media_grid_thw = media_inputs['image_grid_thw'][1:2]
                elif media_type == 'face_vip':
                    # h = int(vip_token_num ** 0.5)
                    # w = int(vip_token_num / h)
                    # media_grid_thw = torch.tensor([[1, h, w]])
                    pass
                else:
                    media_inputs = processor.image_processor(
                        images=None, videos=videos, return_tensors='pt', do_resize=False)
    ...
    def _post_encode(self, model, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {'inputs_embeds': inputs_embeds, 'ori_input_ids': input_ids, 'image_grid_thw': image_grid_thw, 'image_embeds': image_embeds}

register_template(QwenTemplateMeta(MLLMTemplateType.kq_qwen2_5_vl, template_cls=KQ_Qwen2_5VLTemplate))

...
class CustomTuner(Tuner):
    ...
    def prepare_model(args: 'TrainArguments', model: torch.nn.Module) -> torch.nn.Module:
        model_arch = get_model_arch(model.model_meta.model_arch)
        # NOTE:
        target_regex = get_multimodal_target_regex(model, freeze_llm=False, freeze_vit=False, freeze_aligner=False, another_rejected_modules=['facechecker'])
...
def create_custom_optimizer(args, model, dataset):
    ...
    # fc_parameters = [(n, p) for n, p in model.named_parameters()
    #                  if 'facechecker' in n]
    # print(f'vit_parameters: {len(vit_parameters)}, fc_parameters: {len(fc_parameters)}')
    ...
    for n, p in model.named_parameters():
        if 'lora' in n:
            ...
            print(n)
        elif 'facechecker' in n:
            p.requires_grad = True
            fc_parameters.append((n, p))
            print(n)
        else:
    ...
    optimizer_grouped_parameters = [
        {
            'params': [p for n, p in lora_parameters],
            'weight_decay': args.weight_decay,
            'lr': args.learning_rate,
        },
        {
            'params': [p for n, p in fc_parameters],
            'weight_decay': args.weight_decay,
            'lr': args.learning_rate,
        }
        # vit & merger
        ...
        # # facechecker
        # {
        #     'params': [p for n, p in fc_parameters if n in decay_parameters],
        #     'weight_decay': args.weight_decay,
        #     'lr': args.learning_rate,
        # },
